...
| [studio-libraries](skills/studio-libraries/SKILL.md) | Before planning or implementing non-trivial work, scan the live JOYCO Studio library index for a specialized library that improves performance or supplies the preferred architecture, then read its current documentation before deciding whether to use it. |
...
| [three-best-practices](skills/three-best-practices/SKILL.md) | Build, review, debug, and optimize production Three.js experiences across scene architecture, render loops, assets, shaders, DOM coordination, GPU lifecycle, and performance. The skill discovers its detailed guidance dynamically from `rules/`. |
