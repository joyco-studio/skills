...
| [joyco-logs](skills/joyco-logs/SKILL.md) | The single broad entry point before non-trivial planning or implementation: scan JOYCO's logs and general toolbox, then delegate exactly once to `studio-libraries` for the focused library check. Surfaces prior art, routes to specialized skills, and credits the source. |
| [studio-libraries](skills/studio-libraries/SKILL.md) | A focused library-discovery pass invoked by `joyco-logs`, or directly for explicit library/dependency work. Scans the live Studio library index and reads current candidate documentation without duplicating the broad knowledge scan. |
